...
const MONGODB_DATABASE_NODES_DEV = "nodes_dev"
...
func CreateMongoDbClientWithCredentials(dbType AuthType, mongoDbUrl string, mongoDbUsername string, mongoDbPassword string, mongoDbAuthSource string) (*MongoDBClient, error) {
	...
	default:
		return nil, core.CreateErr(nil, nil, "unknown database type: %v", dbType)
	}

	clientOptions := options.Client().ApplyURI(mongoDbUrl).SetAuth(credentials)

	if clientOptions.Validate() != nil {
		return nil, core.CreateErr(nil, nil, "error validating MongoDB client options: %v", clientOptions.Validate())
	}

	client, err := mongo.Connect(ctx, clientOptions)
	if err != nil {
		return nil, core.CreateErr(nil, nil, "error creating MongoDB client: %v", err)
	}
