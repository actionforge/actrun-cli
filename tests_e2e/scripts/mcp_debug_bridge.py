...
async def main():
    root_dir = os.environ.get("ACT_ROOT", os.path.join(os.environ.get("ACT_GRAPH_FILES_DIR", "."), "..", ".."))
    graph_path = os.path.join(root_dir, "examples", "mcp-debug-example.act")
        ...
        async with websockets.connect(f"ws://127.0.0.1:{port}/ws") as websocket:
            async for message in websocket:
                ...
                if msg_type == "control":
                    ...
                    depth = node.count("/")
                    clean_and_print(f"DEBUG PAUSED #{pause_count} at node: {node} (depth={depth})")

                    await asyncio.sleep(0.1)

                    if node == "my-group" and not entered_group:
                        # First time at the group node — step INTO it
                        entered_group = True
                        clean_and_print("Sending STEP_INTO command")
                        await websocket.send(json.dumps({"type": "debug_step_into"}))

                    elif "/" in node:
                        # Inside the group (depth > 0) — step to see inner nodes,
                        # then step OUT on the second inner node
                        clean_and_print("Sending STEP_OUT command")
                        await websocket.send(json.dumps({"type": "debug_step_out"}))

                    elif node == "print-done":
                        # Past the group — resume to finish
